...
| Frontend double-countdown on level 1 | **Needs decision** (non-blocking) |
...
| 3 | **Blocker (fixed)** | Fail-restart path refills HP (`refill_life_for_restart`, ~L1920–1924) **before** any effect phase would run. | Order locked: `fail.led` → stinger → `continue` → countdown (loop top) → refill HP → replay. |
| 4 | **High (fixed)** | Plan suggested `fail.led` for life=0 with ≤10s session time left. GLOBAL_RULES session/game over uses **clear** hold (same as timer expire), not red fail. | `_finish_session()` always plays `clear.led`; red fail only on >10s restart path. |
...
| 8 | **Medium (noted)** | Pre-session `CountdownScreen` (3-2-1-**GO**, 1 s/step) will double-countdown level 1 unless removed or demoted. | Locked recommendation in human decisions. |
...
|---|----------|----------------|
| 1 | **Pre-session UI countdown** — remove vs keep as branding splash | **Remove** (or skip to Simulator immediately on `POST /start-game`) — backend `.led` countdown is single source of truth for floor + UI sync. |
| 2 | **Transition stinger asset** | One stock ~2–3 s mp3 until final asset (per GLOBAL_RULES). |
| 3 | **Last level cleared with time remaining** | Same as mid-session clear: `clear.led` → stinger → **no countdown** → session end black (not a separate win animation). |
| 4 | **Frontend score SFX** | Mute SimulatorScreen synth beeps when backend `SessionAudio` is active (recommended: yes). |
| 5 | **`.led` authoring owner** | On-site level editor vs `scripts/build_effect_led.py` bootstrap — either OK if output validates at 16×26. |
...
| # | Decision |
|---|----------|
| 1 | **Effect visuals = standalone `.led` mini-levels** under `games/source/effects/`, loaded via `_load_level_file()` → `_prepare_level_attempt()` → `Play.running()` (not hard-coded RGB fills). |
...
| 4 | **Countdown before every level start** (first level, after clear, after fail restart). Floor shows green **3-2-1 only** (no GO glyph). Not after session end. |
| 5 | **Native 16×26 effect authoring** — archives authored at full matrix size; `prepare_level_for_platform()` still called for one code path / future retargets. |
...
stateDiagram-v2
    ...
    SessionEndHold --> Stinger: clear.led done
...
**Session end** (timer, life≤10s, sequence exhausted, stop): `clear.led → Stinger → Black` — **no countdown**.
...
def _finish_session(game, audio):
    """Session end: blue clear hold → stinger → black. No countdown."""
    if game.running:
        run_effect_led(clear.led, session_phase="clear", ...)
        audio.play_stinger()          # fire-and-forget; concurrent with clear hold
    game.update_state(session_phase="ended")
...
|------|------------------------------|
| Timer expire mid-level | `_finish_session()` → clear + stinger (no countdown) |
| Life=0 with ≤10 s left | `_finish_session()` → clear + stinger (not fail.red) |
| Pre-loop timeout (~L1860) | `_finish_session()` before result publish |
| Last level cleared | clear + stinger already ran in inner loop → `_finish_session()` only blanks (no second clear) |
| Fail restart (>10 s) | fail + stinger → countdown → refill → replay; no `_finish_session()` |

**First level:** backend countdown at loop entry is authoritative. Frontend must not be the sole countdown source (see human decision #1).
...
2. `game.update_state(led_display=..., session_phase=..., countdown_digit=...)`.
...
Frontend follow-ups (Phase C): drive overlay from polled `/game-state` or WebSocket; show UI **GO** only when `session_phase=playing` (after digit 1 clears on floor, ~0.6 s cosmetic delay OK).
...
| `play_stinger(path)` | Short one-shot on SFX channel; fire-and-forget at hold phase entry |
...
- Lazy-init mixer inside `session_audio` **after** import shim (do not remove shim until then).
...
| B7 | Fail path: fail → stinger → continue → countdown → refill → replay | B4 |
| B8 | Clear path: clear → stinger → break (countdown on next loop iter) | B4 |
...
| All lives lost, time > 10 s | `fail` → stinger → `countdown` → refill → same level reloaded |
| Session timer expires mid-level | `_finish_session`: `clear` → stinger → `ended`; **no** countdown |
| Life=0 with ≤10 s left | `_finish_session`: `clear` (not fail) → stinger → black; **no** countdown |
| Sequence exhausted (last level clear) | `clear` → stinger in loop; `_finish_session` blanks only — **no** second clear, **no** countdown |
...
| Double countdown (UI + backend) | Backend authoritative; remove/demote frontend pre-start countdown |
...
5. Life=0 with ≤10 s session time shows clear (blue) hold, stinger, then black — **no** fail red, **no** countdown.
6. BGM plays only during active gameplay; off during countdown, transitions, clear, fail, and session end.
7. Score SFX fire on backend score changes without blocking the game thread.
8. Effect visuals loaded from `games/source/effects/*.led` via `Play.running()`, not hard-coded fills.
9. All new tests in §6 pass in CI with `DISABLE_AUDIO=1`.
