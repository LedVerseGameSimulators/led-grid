...
def _build_level_sequence(start_level):
    """Ordered list of level FILE PATHS from start_level to series end.
    Floor Is Lava series:
      easy:    source/-/*.led      (001..010)
      medium:  source/--/*.led
      hard:    source/---/*.led
      2player: source/----/*.ledb  (01..13)
    ...
    buckets = [
        ("easy",    os.path.join(src, "source", "-",    "*.led")),
        ("medium",  os.path.join(src, "source", "--",   "*.led")),
        ("hard",    os.path.join(src, "source", "---",  "*.led")),
        ("2player", os.path.join(src, "source", "----", "*.ledb")),
    ]

    files = []
    # Exact stem match first (001 must not match 001 HARD in hard series).
    for _name, pattern in buckets:
        candidates = sorted(_glob.glob(pattern))
        if any(os.path.basename(f).rsplit(".", 1)[0] == sl for f in candidates):
            files = candidates
            break
    # Prefix fallback for partial ids (e.g. "01" → 01.ledb).
    if not files:
        for _name, pattern in reversed(buckets):
            candidates = sorted(_glob.glob(pattern))
            stems = [os.path.basename(f).rsplit(".", 1)[0] for f in candidates]
            if sl in stems or any(s.startswith(sl) for s in stems):
                files = candidates
                break
    if not files:
            ...
            try:
                ...
                def _frame_callback(play_self, dgroup, time_pass, total_pass):
                    ...
                    try:
                        ...
                        # ── GRID CLASSIFICATION ──────────────────────────────
                        # Blue = scoreable (+1). Orange = P2 score (2P only).
                        # RED/DEDUCT = hazard. GREEN = safe platform (shields red).
                        goal_cells  = set()
                        goal2_cells = set()
                        red_cells   = set()
                        deduct_cells = set()
                        ...
                        _P1_COLOR = _GRID_P1_COLOR
                        _P2_COLOR = _GRID_P2_COLOR
                        _GREEN    = (0, 254, 0)
                        ...
                        _scoreable_colors = (_GRID_SCORE_COLORS_2P if game.multiplayer
                                             else _GRID_SCORE_COLORS_1P)
                        ...
                        if USE_SERIAL_HD and _hw_led_control is not None:
                            try:
                                with _hw_serial_lock:
                                    _hw_read_sensors(_hw_layout_type, led_table.state_table)
                            except Exception as _hw_err:
                                logger.warning(f"HW read failed: {_hw_err}")

                        with game.input_lock:
                            state = [row[:] for row in led_table.state_table]
                            if game.multiplayer:
                            ...
                            for i in range(led_table.led_row):
                                for j in range(led_table.led_col):
                                    pressed = state[i][j] or (i, j) in game._sim_pressed
                                    if pressed:
